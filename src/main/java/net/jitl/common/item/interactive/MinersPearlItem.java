...
public class MinersPearlItem extends Item implements IEssenceItem {
    ...
    public @NotNull InteractionResultHolder<ItemStack> use(@NotNull Level worldIn, @NotNull Player playerIn, @NotNull InteractionHand handIn) {
        ...
        if (canTeleport) {
            ClientTools.playLocalSound(SoundEvents.ENDERMAN_TELEPORT, 1.0F, 0.5F);
            ClientTools.playLocalSound(JSounds.MINERS_PEARL.get(), 1.25F, Mth.nextFloat(worldIn.getRandom(), 0.95F, 1.55F));
            for (int i = 0; i < 16; ++i) {
            ...
            ClientTools.playLocalSound(JSounds.MINERS_PEARL.get(), 1.25F, 0.5F);
        }
            ...
            if (tag.getBoolean("teleport")) {
                ...
                if (teleportTimer >= 10) {
                    if (player.level.dimension() == Level.OVERWORLD) {
                        player.teleportTo(player.getX(), world.getHeightmapPos(Heightmap.Types.WORLD_SURFACE_WG, player.blockPosition()).getY(), player.getZ());
                        stack.hurtAndBreak(1, player, (context) -> context.broadcastBreakEvent(player.getUsedItemHand()));
                    } else {
