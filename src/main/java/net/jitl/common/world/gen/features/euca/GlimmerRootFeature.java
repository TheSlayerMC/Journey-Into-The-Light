...
public class GlimmerRootFeature extends Feature<NoFeatureConfig> {
    private static final Direction[] DIRECTIONS = Direction.values();

    public GlimmerRootFeature(Codec<NoFeatureConfig> codec_) {
        super(codec_);
    }
    ...
    private void placeOnRoof(IWorld world, Random random, BlockPos blockPos) {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            if (world.isEmptyBlock(mutable)) {
                ...
                if (blockstate.is(JBlocks.GOLDITE_STONE)) {
                    int j = MathHelper.nextInt(random, 1, 10);
