...
public class EntityBouncingProjectile extends EntityThrowable {
    ...
    protected void onImpact(RayTraceResult par1) {
        ...
        if (this.bounces == maxBounces)
            this.world.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, this.posX + f, this.posY + 0.0D + f1, this.posZ + f2, 0.0D, 0.0D, 0.0D);
        else
            this.world.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, this.posX + f, this.posY + 0.0D + f1, this.posZ + f2, 0.0D, 0.0D, 0.0D);
    }
}