...
public class WorldGenFrozenTree4 extends WorldGenHugeTrees {
    ...
    public WorldGenFrozenTree4(boolean notify, boolean p_i45457_2_) {
        ...
        if (!this.ensureGrowable(worldIn, rand, position, i)) {
            ...
            for (int j = 0; j < i; ++j) {
                if (isAirLeaves(worldIn, position.up(j))) {
                    this.setBlockAndNotifyAdequately(worldIn, position.up(j), TRUNK);
                }

                if (j < i - 1) {
                    if (isAirLeaves(worldIn, position.add(1, j, 0))) {
                        this.setBlockAndNotifyAdequately(worldIn, position.add(1, j, 0), TRUNK);
                    }

                    if (isAirLeaves(worldIn, position.add(1, j, 1))) {
                        this.setBlockAndNotifyAdequately(worldIn, position.add(1, j, 1), TRUNK);
                    }

                    if (isAirLeaves(worldIn, position.add(0, j, 1))) {
                        this.setBlockAndNotifyAdequately(worldIn, position.add(0, j, 1), TRUNK);
                    }
                }
            }
