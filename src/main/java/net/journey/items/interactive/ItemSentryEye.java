...
public class ItemSentryEye extends JItem {
	@Override
	public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		IBlockState iblockstate = worldIn.getBlockState(pos);
		ItemStack itemstack = player.getHeldItem(hand);
		if (player.canPlayerEdit(pos.offset(facing), facing, itemstack) && iblockstate.getBlock() == JourneyBlocks.senterianPortalFrame && !iblockstate.getValue(BlockSenterianPortalFrame.EYE)) {
			if (!worldIn.isRemote) {
				worldIn.setBlockState(pos, iblockstate.withProperty(BlockSenterianPortalFrame.EYE, true), 2);
