...
public class ItemAncientPiece extends JItem {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
	    ...
	    if (player.canPlayerEdit(pos.offset(facing), facing, itemstack) && iblockstate.getBlock() == JourneyBlocks.ANCIENT_SOCKET && !iblockstate.getValue(BlockAncientSocket.INSERT)) {
		    if (!worldIn.isRemote) {
			    worldIn.setBlockState(pos, iblockstate.withProperty(BlockAncientSocket.INSERT, true));
