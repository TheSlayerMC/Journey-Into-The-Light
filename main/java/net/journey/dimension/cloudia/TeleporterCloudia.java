...
public class TeleporterCloudia extends Teleporter {
    ...
    protected final Long2ObjectMap<TeleporterCloudia.PortalPosition> destinationCoordinateCache = new Long2ObjectOpenHashMap<TeleporterCloudia.PortalPosition>(4096);
	protected final List destinationCoordinateKeys = new ArrayList();
	protected int dimensionID;
	protected Block portal, frameBlock;

	public TeleporterCloudia(WorldServer worldServer, int dimensionID, Block portal, Block frameBlock) {
		super(worldServer);
	...
	public TeleporterCloudia(WorldServer worldServer) {
		super(worldServer);
		this.worldServerInstance = worldServer;
		this.random = new Random(worldServer.getSeed());
	}
	...
	public boolean placeInExistingPortal(Entity e, float par2) {
		...
		if (this.destinationCoordinateCache.containsKey(k)) {
			...
			for (int l = -128; l <= 128; ++l) {
				...
				for (int i1 = -128; i1 <= 128; ++i1) {
					for (BlockPos blockpos = blockpos4.add(l, 5 - 1 - blockpos4.getY(), i1); blockpos.getY() >= 0; blockpos = blockpos1) {
		...
		if (d0 >= 0.0D) {
			if (flag1) {
                this.destinationCoordinateCache.put(k, new TeleporterCloudia.PortalPosition(object, this.world.getTotalWorldTime()));
			}
			...
			if (enumfacing != null) {
				...
				boolean flag2 = this.checkAir(blockpos2);
				boolean flag3 = this.checkAir(blockpos2.offset(enumfacing2));
				...
				if (flag3 && flag2) {
					...
					flag2 = this.checkAir(blockpos3);
					flag3 = this.checkAir(blockpos3.offset(enumfacing2));
				}
	...
	private boolean checkAir(BlockPos b) {
		return !this.worldServerInstance.isAirBlock(b) || !this.worldServerInstance.isAirBlock(b.up());
	}
			...
			for (i3 = -1; i3 <= 1; ++i3) {
				for (j3 = 1; j3 < 3; ++j3) {
					for (k3 = -1; k3 < 3; ++k3) {
						...
						j4 = k2 + (j3 - 1) * l2 - i3 * l5;
						flag = k3 < 0;
						if(this.worldServerInstance.getBlockState(new BlockPos(l3, i4 - 1, j4)) == JourneyBlocks.cloudiaGrass) {
