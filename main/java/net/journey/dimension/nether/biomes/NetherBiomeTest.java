...
public class NetherBiomeTest extends NetherBiome {
	public NetherBiomeTest(String name) {
		super(name);
	}
	...
	public void genFloorObjects(Chunk chunk, BlockPos pos, Random random) {
		if (random.nextFloat() <= plantDensity && chunk.getBlockState(pos).getBlock() == Blocks.GRAVEL
			...
			if (JourneyBlocks.blazierBricks != Blocks.AIR && random.nextInt(8) == 0) {
				...
				for (int i = 1; i < h; i++)
					chunk.setBlockState(pos.up(i), JourneyBlocks.blazierBricks.getDefaultState());
				chunk.setBlockState(pos.up(h), JourneyBlocks.blazierBricks.getDefaultState());
			} else if (random.nextInt(7) == 0) {
				if (JourneyBlocks.brittleIce != Blocks.AIR && random.nextBoolean())
					chunk.setBlockState(pos.up(), JourneyBlocks.brittleIce.getDefaultState());
				else if (JourneyBlocks.celestiumBlock != Blocks.AIR)
					chunk.setBlockState(pos.up(), JourneyBlocks.celestiumBlock.getDefaultState());
			}
		}
	}
}
