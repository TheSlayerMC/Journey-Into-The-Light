...
public class BlockFrozenPortal extends BlockBreakable {
	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entity) {
		if ((entity.getRidingEntity() == null) && ((entity instanceof EntityPlayerMP))) {
			...
			//thePlayer.triggerAchievement(JourneyAchievements.achievementFrozen);
	...
	public boolean makePortal(World worldIn, BlockPos p) {
		BlockFrozenPortal.Size size = new BlockFrozenPortal.Size(worldIn, p, EnumFacing.Axis.X);
		if(size.isValid() && size.portalBlockCount == 0) {
			size.placePortalBlocks();
			return true;
		} else {
			BlockFrozenPortal.Size size1 = new BlockFrozenPortal.Size(worldIn, p, EnumFacing.Axis.Z);
	...
	public static class Size {
		...
		protected int getDistanceUntilEdge(BlockPos pos, EnumFacing facing) {
			...
			for(i = 0; i < 22; ++i) {
				...
				if(!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock()) || this.world.getBlockState(blockpos.down()).getBlock() != JourneyBlocks.frozenPortalFrame) {
					break;
				}
			}
		...
		for(int j = 0; j < this.width; ++j) {
			if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != JourneyBlocks.frozenPortalFrame) {
				this.height = 0;
				break;
			}
		}
		...
		protected boolean isEmptyBlock(Block blockIn) {
			return blockIn.getMaterial(blockIn.getDefaultState()) == Material.AIR || blockIn == JourneyBlocks.fire || blockIn == JourneyBlocks.frozenPortal;
		}
		...
		public void placePortalBlocks() {
			for(int i = 0; i < this.width; ++i) {
				...
				for(int j = 0; j < this.height; ++j) {
					this.world.setBlockState(blockpos.up(j), JourneyBlocks.frozenPortal.getDefaultState().withProperty(BlockFrozenPortal.AXIS, this.axis), 2);
				}
			}
		}
	}
