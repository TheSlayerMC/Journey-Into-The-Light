...
public class BlockGoldenPortal extends BlockBreakable {
	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entity) {
		if ((entity.getRidingEntity() == null) && ((entity instanceof EntityPlayerMP))) {
			...
			//thePlayer.triggerAchievement(JourneyAchievements.achievementGolden);
	...
	public boolean makePortal(World worldIn, BlockPos p) {
		BlockGoldenPortal.Size size = new BlockGoldenPortal.Size(worldIn, p, EnumFacing.Axis.X);
		if(size.isValid() && size.portalBlockCount == 0) {
			size.placePortalBlocks();
			return true;
		} else {
			BlockGoldenPortal.Size size1 = new BlockGoldenPortal.Size(worldIn, p, EnumFacing.Axis.Z);
	...
	public static class Size {
		...
		protected int getDistanceUntilEdge(BlockPos pos, EnumFacing facing) {
			...
			for(i = 0; i < 22; ++i) {
				...
				if(!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock()) || this.world.getBlockState(blockpos.down()).getBlock() != JourneyBlocks.goldenPortalFrame) {
					break;
				}
			}
		...
		for(int j = 0; j < this.width; ++j) {
			if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != JourneyBlocks.goldenPortalFrame) {
				this.height = 0;
				break;
			}
		}
		...
		protected boolean isEmptyBlock(Block blockIn) {
			return blockIn.getMaterial(blockIn.getDefaultState()) == Material.AIR || blockIn == JourneyBlocks.fire || blockIn == JourneyBlocks.goldenPortal;
		}
		...
		public void placePortalBlocks() {
			for(int i = 0; i < this.width; ++i) {
				...
				for(int j = 0; j < this.height; ++j) {
					this.world.setBlockState(blockpos.up(j), JourneyBlocks.goldenPortal.getDefaultState().withProperty(BlockGoldenPortal.AXIS, this.axis), 2);
				}
			}
		}
	}
