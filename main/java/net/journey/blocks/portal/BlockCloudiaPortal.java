...
public class BlockCloudiaPortal extends BlockModPortal {

	public BlockCloudiaPortal(String name) {
		super(name, "Cloudia Portal");
	}
	...
	public boolean makePortal(World worldIn, BlockPos p) {
		...
		BlockCloudiaPortal.Size size = new BlockCloudiaPortal.Size(worldIn, p, EnumFacing.Axis.X);
	...
	public static class Size {
		...
		protected int getDistanceUntilEdge(BlockPos pos, EnumFacing facing) {
			...
			for(i = 0; i < 22; ++i) {
				...
				if(!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock()) || this.world.getBlockState(blockpos.down()).getBlock() != JourneyBlocks.cloudiaPortalFrame) {
					break;
				}
			}
		...
		for(int j = 0; j < this.width; ++j) {
			if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != JourneyBlocks.cloudiaPortalFrame) {
				this.height = 0;
				break;
			}
		}
		...
		protected boolean isEmptyBlock(Block blockIn) {
			return blockIn.getMaterial(blockIn.getDefaultState()) == Material.AIR || blockIn == JourneyBlocks.fire || blockIn == JourneyBlocks.cloudiaPortal;
		}
		...
		public void placePortalBlocks() {
			for(int i = 0; i < this.width; ++i) {
				...
				for(int j = 0; j < this.height; ++j) {
					this.world.setBlockState(blockpos.up(j), JourneyBlocks.cloudiaPortal.getDefaultState().withProperty(BlockCloudiaPortal.AXIS, this.axis), 2);
				}
			}
		}
	}
