...
public abstract class EntityBloodKnife extends EntityThrowable implements IProjectile {
	...
	private static final DataParameter<Byte> CRITICAL = EntityDataManager.<Byte>createKey(EntityBloodKnife.class,
	...
	public EntityBloodKnife.PickupStatus pickupStatus;
	...
	public EntityBloodKnife(World worldIn) {
		super(worldIn);
		this.xTile = -1;
		this.yTile = -1;
		this.zTile = -1;
		this.pickupStatus = EntityBloodKnife.PickupStatus.DISALLOWED;
		this.setSize(0.5F, 0.5F);
	}

	public EntityBloodKnife(World worldIn, double x, double y, double z, double dam) {
		this(worldIn);
		this.damage = dam;
		this.setPosition(x, y, z);
	}

	public EntityBloodKnife(World worldIn, EntityLivingBase shooter, float dam) {
		this(worldIn, shooter.posX, shooter.posY + (double) shooter.getEyeHeight() - 0.10000000149011612D,
				shooter.posZ, dam);
		this.shootingEntity = shooter;
		this.damage = dam;
		if (shooter instanceof EntityPlayer) {
			this.pickupStatus = EntityBloodKnife.PickupStatus.ALLOWED;
		}
	}
		...
		if (entity != null) {
			...
			if (entity.attackEntityFrom(damagesource, (float) i)) {
				...
				if (!this.world.isRemote && this.motionX * this.motionX + this.motionY * this.motionY
					...
					if (this.pickupStatus == EntityBloodKnife.PickupStatus.ALLOWED) {
	...
	public void readEntityFromNBT(NBTTagCompound compound) {
		...
		if (compound.hasKey("pickup", 99)) {
			this.pickupStatus = EntityBloodKnife.PickupStatus.getByOrdinal(compound.getByte("pickup"));
		} else if (compound.hasKey("player", 99)) {
			this.pickupStatus = compound.getBoolean("player") ? EntityBloodKnife.PickupStatus.ALLOWED
					: EntityBloodKnife.PickupStatus.DISALLOWED;
		}
	...
	public void onCollideWithPlayer(EntityPlayer entityIn) {
		if (!this.world.isRemote && this.inGround && this.arrowShake <= 0) {
			boolean flag = this.pickupStatus == EntityBloodKnife.PickupStatus.ALLOWED
					|| this.pickupStatus == EntityBloodKnife.PickupStatus.CREATIVE_ONLY
	...
	protected ItemStack getDropStack() {
		return new ItemStack(JourneyItems.bloodKnife);
	}
	...
	public static enum PickupStatus {
		...
		public static EntityBloodKnife.PickupStatus getByOrdinal(int ordinal) {
