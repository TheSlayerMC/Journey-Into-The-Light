...
public abstract class EntityAquaticKnife extends EntityThrowable implements IProjectile {
	...
	private static final DataParameter<Byte> CRITICAL = EntityDataManager.<Byte>createKey(EntityAquaticKnife.class,
	...
	public EntityAquaticKnife.PickupStatus pickupStatus;
	...
	public EntityAquaticKnife(World worldIn) {
		super(worldIn);
		this.xTile = -1;
		this.yTile = -1;
		this.zTile = -1;
		this.pickupStatus = EntityAquaticKnife.PickupStatus.DISALLOWED;
		this.setSize(0.5F, 0.5F);
	}

	public EntityAquaticKnife(World worldIn, double x, double y, double z, double dam) {
		this(worldIn);
		this.damage = dam;
		this.setPosition(x, y, z);
	}

	public EntityAquaticKnife(World worldIn, EntityLivingBase shooter, float dam) {
		this(worldIn, shooter.posX, shooter.posY + (double) shooter.getEyeHeight() - 0.10000000149011612D,
				shooter.posZ, dam);
		this.shootingEntity = shooter;
		this.damage = dam;
		if (shooter instanceof EntityPlayer) {
			this.pickupStatus = EntityAquaticKnife.PickupStatus.ALLOWED;
		}
	}
		...
		if (entity != null) {
			...
			if (entity.attackEntityFrom(damagesource, (float) i)) {
				...
				if (!this.world.isRemote && this.motionX * this.motionX + this.motionY * this.motionY
					...
					if (this.pickupStatus == EntityAquaticKnife.PickupStatus.ALLOWED) {
	...
	public void readEntityFromNBT(NBTTagCompound compound) {
		...
		if (compound.hasKey("pickup", 99)) {
			this.pickupStatus = EntityAquaticKnife.PickupStatus.getByOrdinal(compound.getByte("pickup"));
		} else if (compound.hasKey("player", 99)) {
			this.pickupStatus = compound.getBoolean("player") ? EntityAquaticKnife.PickupStatus.ALLOWED
					: EntityAquaticKnife.PickupStatus.DISALLOWED;
		}
	...
	public void onCollideWithPlayer(EntityPlayer entityIn) {
		if (!this.world.isRemote && this.inGround && this.arrowShake <= 0) {
			boolean flag = this.pickupStatus == EntityAquaticKnife.PickupStatus.ALLOWED
					|| this.pickupStatus == EntityAquaticKnife.PickupStatus.CREATIVE_ONLY
	...
	public static enum PickupStatus {
		...
		public static EntityAquaticKnife.PickupStatus getByOrdinal(int ordinal) {
