...
public abstract class EntitySizzlingKnife extends EntityThrowable implements IProjectile {
	...
	private static final DataParameter<Byte> CRITICAL = EntityDataManager.<Byte>createKey(EntitySizzlingKnife.class,
	...
	public EntitySizzlingKnife.PickupStatus pickupStatus;
	...
	public EntitySizzlingKnife(World worldIn) {
		...
		this.yTile = -1;
		this.zTile = -1;
		this.pickupStatus = EntitySizzlingKnife.PickupStatus.DISALLOWED;
		this.setSize(0.5F, 0.5F);
	}

	public EntitySizzlingKnife(World worldIn, double x, double y, double z, double dam) {
		this(worldIn);
		this.damage = dam;
		this.setPosition(x, y, z);
	}

	public EntitySizzlingKnife(World worldIn, EntityLivingBase shooter, float dam) {
		this(worldIn, shooter.posX, shooter.posY + (double) shooter.getEyeHeight() - 0.10000000149011612D,
				shooter.posZ, dam);
		this.shootingEntity = shooter;
		this.damage = dam;
		if (shooter instanceof EntityPlayer) {
			this.pickupStatus = EntitySizzlingKnife.PickupStatus.ALLOWED;
		}
	}
		...
		if (entity != null) {
			...
			if (entity.attackEntityFrom(damagesource, (float) i)) {
				...
				if (!this.world.isRemote && this.motionX * this.motionX + this.motionY * this.motionY
					...
					if (this.pickupStatus == EntitySizzlingKnife.PickupStatus.ALLOWED) {
	...
	public void readEntityFromNBT(NBTTagCompound compound) {
		...
		if (compound.hasKey("pickup", 99)) {
			this.pickupStatus = EntitySizzlingKnife.PickupStatus.getByOrdinal(compound.getByte("pickup"));
		} else if (compound.hasKey("player", 99)) {
			this.pickupStatus = compound.getBoolean("player") ? EntitySizzlingKnife.PickupStatus.ALLOWED
					: EntitySizzlingKnife.PickupStatus.DISALLOWED;
		}
	...
	public void onCollideWithPlayer(EntityPlayer entityIn) {
		if (!this.world.isRemote && this.inGround && this.arrowShake <= 0) {
			boolean flag = this.pickupStatus == EntitySizzlingKnife.PickupStatus.ALLOWED
					|| this.pickupStatus == EntitySizzlingKnife.PickupStatus.CREATIVE_ONLY
	...
	protected ItemStack getDropStack() {
		return new ItemStack(JourneyItems.sizzlingKnife);
	}
	...
	public static enum PickupStatus {
		...
		public static EntitySizzlingKnife.PickupStatus getByOrdinal(int ordinal) {
